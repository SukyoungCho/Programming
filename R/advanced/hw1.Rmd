...
```{r}
#a Create a scatterplot of `farm` vs. `land`
plot(y = area$farm, x = area$land)
abline(lm(farm ~ land, data = area), col = "limegreen")

#b Fix $k=19000$. Estimate $\beta_0$ and $\beta_1$ using the Nelder-Mead method in `optim()`
robust.regr <- function(x) {
  b0 <- x[1]
  b1 <- x[2]
  x <- area$land
  y <- area$farm
  ...
  k <- 19000
  t <- (y - b0 - (b1*x))
  value <- ifelse((abs(t) <= k), (t^2), (2*k*abs(t)-(k^2)))
  return((sum(value)/n))
}
nd <- optim(par = c(0,0), fn = robust.regr, method = "Nelder-Mead")
plot(y = area$farm, x = area$land)
abline(lm(farm ~ land, data = area), col = "limegreen")
abline(a = nd$par[1], b = nd$par[2], col = "navy")

#c Change the method to BFGS to get another set of estimates.
# Add this line to your plot colored "black".
d.robust.regr <- function(x) {
  b0 <- x[1]
  b1 <- x[2]
  x <- area$land
  y <- area$farm
  ...
  k <- 19000
  t <- (y - b0 - (b1*x))
  d.db <- ifelse((abs(t) <= k), (2*t), (2*k*sign(t)))
  return(c(-(sum(d.db)/n), -(sum(x*d.db)/n)))
}
bfgs <- optim(par = c(0,0), fn = robust.regr, gr = d.robust.regr, method = "BFGS")
plot(y = area$farm, x = area$land)
abline(lm(farm ~ land, data = area), col = "limegreen")
abline(a = nd$par[1], b = nd$par[2], col = "navy")
abline(a = bfgs$par[1], b = bfgs$par[2], col = "black")

#d Change the method to CG to get another set of estimates. Add this
# line to your plot colored "coral" and with line type "dashed"
cg <- optim(par = c(0,0), fn = robust.regr, gr = d.robust.regr, method = "CG")
plot(y = area$farm, x = area$land)
abline(lm(farm ~ land, data = area), col = "limegreen")
abline(a = nd$par[1], b = nd$par[2], col = "navy")
abline(a = bfgs$par[1], b = bfgs$par[2], col = "black")
abline(a = cg$par[1], b = cg$par[2], col = "coral", lty = "dashed")
...
abline(a = bfgs$par[1], b = bfgs$par[2], col = "black")
abline(a = cg$par[1], b = cg$par[2], col = "coral", lty = "dashed")
legend("topleft", legend = c("lm", "ND", "BFGS", "CG"), lty = c(1, 1, 1, 2), col = c("limegreen", "navy", "black", "coral"))
```
...
y.hat <- function(y, b){
  y.hat.val = y[1]
  for (i in 2:length(y)) {
    y.hat.val[i] = (b*y[i-1]) + ((1-b)*y.hat.val[i-1])
  }
  return(y.hat.val)
}
forecast.error <- function(b) {
  y = nhtemp
  return(sum((y - y.hat(y, b))^2)/60)
}
fore <- optimize(forecast.error, interval = c(0,1), lower = min(c(0,1), upper = max(c(0,1))))
fore$minimum     
```
...
```{r}
y.hat.val <- y.hat(y = nhtemp, b= fore$minimum)
plot(nhtemp)
lines(x = 1912:1971, y = y.hat.val, col = "blue") 
```
...
```{r}
y.hat.val <- y.hat(y = nhtemp, b= fore$minimum)
y.hat.1 <- y.hat(y = nhtemp, b= .1)
y.hat.9 <- y.hat(y = nhtemp, b= .9)
plot(nhtemp)
lines(x = 1912:1971, y = y.hat.val, col = "blue") 
lines(x = 1912:1971, y = y.hat.1, col = "red")
lines(x = 1912:1971, y = y.hat.9, col = "green")
legend("bottomright", legend = c("Y", "Min", ".1", ".9"), lty = c(1, 1, 1, 1), col = c("black" ,"blue", "red", "green"))
...
mle <- function(par, x) {
  ...
  x = mtcars$mpg
  n = length(x)
  return(-(n*log(1/(sqrt(2*pi)*sigma)) - ((1/(2*(sigma)^2))*sum((x-mu)^2))))
}
optim(c(0, 1), mle, method = "Nelder-Mead")
mean(mtcars$mpg)
sd(mtcars$mpg)
```
...
```{r}
mle.lm <- function(b, x, y) {
  b0 <- b[1]
  b1 <- b[2]
  return(-((-sum(log(1 + exp(b0 + (b1*x))))) + (sum(y*(b0 + (b1*x))))))
}
mle.b0.b1 <- optim(par = c(0,0), fn = mle.lm, method = "Nelder-Mead", x = menarche.cases$age, y = menarche.cases$reached.menarche)
mle.b0.b1$par
p.b0.b1 <- function(x) {
  return((1/(1 + exp(-(mle.b0.b1$par[1] + (mle.b0.b1$par[2]*x))))))
}
plot(jitter(x = menarche.cases$age), y = jitter(menarche.cases$reached.menarche))
points(x = menarche$Age, y = (menarche$Menarche/menarche$Total), col  = "red")
curve(p.b0.b1, from = min(menarche.cases$age), to = max(menarche.cases$age), col="green", add = TRUE)
```
