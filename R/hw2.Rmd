...
monte.carlo.pi = function(n, draw=FALSE) {
  ...
  count = 0
  for(i in 1:n){
    if (points[i] < 1) { #count the number of points inside the circle
      count = count + 1
    } else {
      count = count + 0
    }
  ...
  if (draw == TRUE) {
    plot <- plot(x,y) + lines(x = cos(seq(from = 0, to = 2*pi, by = .01)), y = sin(seq(from = 0, to = 2*pi, by = .01))) # plot a graph with a line of circle
  }
  return(pi = 4*(count/n))
}
...
```{r}
# ... your code here
pi.estimate.100 <- replicate(500, expr = monte.carlo.pi(100))
pi.estimate.1000 <- replicate(500, expr = monte.carlo.pi(1000))
par(mfrow = c(2, 1)) # to draw 2 plots
hist(pi.estimate.100, breaks = 20, main = "Estimate of pi with n = 100")
hist(pi.estimate.1000, breaks = 20, main = "Estimate of pi with n = 1000")
...
scores$Grade <- mapply(FUN = CourseGrade, scores$Pct.gr, scores$Pctile.gr, USE.NAMES = FALSE)
...
```{r}
# ... your code here
test <- tapply(X = scores$E1, INDEX = scores$Grade, FUN = mean)
round(test)
```
...
```{r}
# ... your code here
apply(X = scores[,1, drop = FALSE], MARGIN = 2, FUN = mean) #Quiz 1
apply(X = scores[,2, drop = FALSE], MARGIN = 2, FUN = mean) #Quiz 2
apply(X = scores[,3, drop = FALSE], MARGIN = 2, FUN = mean) #Quiz 3
apply(X = scores[,4, drop = FALSE], MARGIN = 2, FUN = mean) #Quiz 4
...
```{r}
# ... your code here
apply(X = scores[,1:4, drop = FALSE], MARGIN = 1, FUN = mean)
